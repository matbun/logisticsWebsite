...
if ($stmt = mysqli_prepare($link, "SELECT user_id, username, type, developer FROM company_users WHERE username = ? AND password = sha1(?)")) {
    ...
    mysqli_stmt_bind_result($stmt, $usr_id, $usr_name, $usr_type, $developer);
...
if(isset($usr_id)){
	...
	$_SESSION['user_info'] = serialize(new UserInfo($usr_id, $usr_name, $usr_type, $developer));
